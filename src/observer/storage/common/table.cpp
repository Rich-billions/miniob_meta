...
RC Table::delete_all_index(const std::string &path, const char *table_name) {
  for(const auto &idx : indexes_) {
    const char *index_name = idx->index_meta().name();
    //std::string idx_file_path = path + "/" + table_name + "-" + index_name +TABLE_INDEX_SUFFIX;
    const std::string idx_file_path = table_index_file(path.c_str(), table_name, index_name);
    if(remove(idx_file_path.c_str()) != 0) {
      return RC::IOERR;
    }
  }
  return RC::SUCCESS;
