...
RC Db::drop_table(const char *table_name) {
  ...
  if(table->delete_all_index(path_, table_name) != RC::SUCCESS) {
    return RC::IOERR;
  }

  //delete all table_meta files
  const std::string table_meta_file_path = table_meta_file(path_.c_str(), table_name);
  if(remove(table_meta_file_path.c_str()) != 0) {
    return RC::IOERR;
  }

  //delete all data files
  const std::string table_data_file_path = table_data_file(path_.c_str(), table_name);
